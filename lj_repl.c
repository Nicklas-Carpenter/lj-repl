...
static void write_prompt(lua_State *L, int firstline, char *prompt)
{
  const char *p;
  lua_getfield(L, LUA_GLOBALSINDEX, firstline ? "_PROMPT" : "_PROMPT2");
  p = lua_tostring(L, -1);
  if (p == NULL) p = firstline ? LUA_PROMPT : LUA_PROMPT2;
  strncpy(prompt, p, LUA_MAXINPUT);
...
{
  char prompt[LUA_MAXINPUT];
  char *line = NULL;

  write_prompt(L, firstline, prompt);
